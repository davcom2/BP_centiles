...
bootstapping <- function(data, model) {
  ...
  BOOT_CI <- function(){
    ...
    newx<-seq(20,90,2)
    mat <- centiles.pred(boot_model, xname="age", xvalues=newx, cent=c(1,5,10,25,50,75,90,95,99), data=data_samp )
    id <- i
    mat <- cbind(id=id, mat)
...
lmsdfj <- function(data_samp) {gamlss(DBP ~ fp( age, 3), sigma.fo=~ fp( age, 3),family=ST3,data=data_samp,n.cyc=200)}
...
doc <- docx( )%>%
  addTitle("Female DBP centiles with 95% CI") %>%
  addFlexTable(table %>%
  ...
  writeDoc(doc, file = "DBP_female_ci_101018.docx")


dbp_f_table <- table
save(dbp_f_table,file="C:/Users/sgerry/Dropbox/Steve work/VM/Precursor/DBP_f_table.RData")
