...
lmsdfj <- function(data_samp) {lms(pp, age, data=data_samp,trans.x=TRUE, families=c("ST3"), n.cyc=100)}
...
doc <- docx( )%>%
  addTitle("Female PP centiles with 95% CI") %>%
  addFlexTable(table %>%
  ...
  writeDoc(doc, file = "PP_female_ci_111018.docx")

pp_f_table <- table
save(pp_f_table,file="C:/Users/sgerry/Dropbox/Steve work/VM/Precursor/PP_f_table.RData")
