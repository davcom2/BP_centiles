...
pp_ST31 <- lms(pp, age, data=dat,trans.x=TRUE, families=c("ST3"), n.cyc=100)

mat1 <- centiles.pred(pp_ST31, xname="age",xvalues=seq(18,90,0.1), plot=TRUE, cent=seq(1,99,1), legend=F, xlab="Age (years)", ylab="Systolic Blood Pressure (mmHg)" )
mat1t <- centiles.pred(pp_ST31, xname="age",xvalues=seq(20,90,2), plot=F, cent=c(1,5,10,25,50,75,90,95,99))
...
empirical1<-ddply(dat, "year", here(summarise), P = quantile(pp, .01))
empirical5<-ddply(dat, "year", here(summarise), P = quantile(pp, .05))
empirical10<-ddply(dat, "year", here(summarise), P = quantile(pp, .10))
empirical25<-ddply(dat, "year", here(summarise), P = quantile(pp, .25))
empirical50<-ddply(dat, "year", here(summarise), P = quantile(pp, .50))
empirical75<-ddply(dat, "year", here(summarise), P = quantile(pp, .75))
empirical90<-ddply(dat, "year", here(summarise), P = quantile(pp, .90))
empirical95<-ddply(dat, "year", here(summarise), P = quantile(pp, .95))
empirical99<-ddply(dat, "year", here(summarise), P = quantile(pp, .99))
...
pp_ST32 <- lms(pp, age, data=dat,trans.x=TRUE, families=c("ST3"), n.cyc=100)

mat2 <- centiles.pred(pp_ST32, xname="age",xvalues=seq(18,90,0.1), plot=TRUE, cent=seq(1,99,1), legend=F, xlab="Age (years)", ylab="Systolic Blood Pressure (mmHg)" )
mat2t <- centiles.pred(pp_ST32, xname="age",xvalues=seq(20,90,2), plot=F, cent=c(1,5,10,25,50,75,90,95,99))
...
g <- g + theme_bw() + scale_x_continuous(breaks = seq(20,90,10), limits=c(20,90)) + 
  ...
  theme(legend.position="none") + ylab("Pulse Pressure (mmHg)") + xlab("Age (years)") + ggtitle("Male")
...
g1 <- g1 + theme_bw() + scale_x_continuous(breaks = seq(20,90,10), limits=c(20,90)) + 
  scale_y_continuous(breaks = c(20,40,60,80,100,120,140,140), limits=c(20,140)) + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + 
  theme(legend.position="none") + ylab("Pulse Pressure (mmHg)") + xlab("Age (years)") + ggtitle("Female")
...
writeDoc(doc, file = "PP_main_091018_100.docx")
...
save(pp_m,pp_f,pp_m.e,pp_f.e,file="C:/Users/sgerry/Dropbox/Steve work/VM/Precursor/PP_plots.RData")
