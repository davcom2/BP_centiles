...
#DBP_ST31 <- lms(DBP, age, data=dat,trans.x=TRUE, families=c("ST3"), n.cyc=100)

DBP_ST3_FP1 <- gamlss(DBP ~ fp( age, 3), sigma.fo=~ fp( age, 3),family=ST3,data=dat,n.cyc=200)

mat1 <- centiles.pred(DBP_ST3_FP1, xname="age",xvalues=seq(18,90,0.1), plot=TRUE, cent=seq(1,99,1), legend=F, xlab="Age (years)", ylab="Systolic Blood Pressure (mmHg)" )
mat1t <- centiles.pred(DBP_ST3_FP1, xname="age",xvalues=seq(20,90,2), plot=F, cent=c(1,5,10,25,50,75,90,95,99))
...
empirical10<-ddply(dat, "year", here(summarise), P = quantile(DBP, .10))
empirical25<-ddply(dat, "year", here(summarise), P = quantile(DBP, .25))
empirical50<-ddply(dat, "year", here(summarise), P = quantile(DBP, .50))
empirical75<-ddply(dat, "year", here(summarise), P = quantile(DBP, .75))
empirical90<-ddply(dat, "year", here(summarise), P = quantile(DBP, .90))
...
#DBP_ST32 <- lms(DBP, age, data=dat,trans.x=TRUE, families=c("ST3"), n.cyc=100)

DBP_ST3_FP2 <- gamlss(DBP ~ fp( age, 3), sigma.fo=~ fp( age, 3),family=ST3,data=dat,n.cyc=200)

mat2 <- centiles.pred(DBP_ST3_FP2, xname="age",xvalues=seq(18,90,0.1), plot=TRUE, cent=seq(1,99,1), legend=F, xlab="Age (years)", ylab="Systolic Blood Pressure (mmHg)" )
mat2t <- centiles.pred(DBP_ST3_FP2, xname="age",xvalues=seq(20,90,2), plot=F, cent=c(1,5,10,25,50,75,90,95,99))
...
g <- g + theme_bw() + scale_x_continuous(breaks = seq(20,90,10), limits=c(20,90)) + 
  ...
  theme(legend.position="none") + ylab("Diastolic Blood Pressure (mmHg)") + xlab("Age (years)") + ggtitle("Male")
...
g1 <- g1 + theme_bw() + scale_x_continuous(breaks = seq(20,90,10), limits=c(20,90)) + 
  scale_y_continuous(breaks = c(40,60,80,100,120), limits=c(40,120)) + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + 
  theme(legend.position="none") + ylab("Diastolic Blood Pressure (mmHg)") + xlab("Age (years)") + ggtitle("Female")
...
writeDoc(doc, file = "DBP_main_091018_100_FP.docx")
...
save(dbp_m,dbp_f,dbp_m.e,dbp_f.e,file="C:/Users/sgerry/Dropbox/Steve work/VM/Precursor/DBP_plots.RData")